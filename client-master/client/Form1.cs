...
namespace client
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            InitializeComponent();
            //Leavegame_Button.Enabled = false;
        }
        ...
        private void leaveGameButton_Click(object sender, EventArgs e)
        {
            if (connected)
            {
                // Send a spectate request to the server
                string message = "SPECTATE";
                Byte[] buffer = Encoding.Default.GetBytes(message);
                clientSocket.Send(buffer);

                // Display a message locally that the player is now spectating
                logs.AppendText("You are now spectating.\n");

                // Disable game interaction buttons as the player is now spectating
        ...
        private void disconnectButton_Click(object sender, EventArgs e)
        {
            if (connected)
            {
                ...
                // Display a message locally that the player is now spectating
                logs.AppendText("You are now leaving.\n");
